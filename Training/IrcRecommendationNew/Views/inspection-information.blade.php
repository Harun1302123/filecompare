@if(count($inspectionInfo) > 0 && ((Auth::user()->user_type == '5x505' && in_array($appInfo->status_id,[15, 16, 25])) || in_array(Auth::user()->user_type, ['1x101','2x202', '4x404'])))
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        @foreach($inspectionInfo as $inspectionInfo)
                            ...
                            @if($inspectionInfo->created_by == Auth::user()->id || in_array(Auth::user()->user_type, ['1x101','2x202']) || (Auth::user()->user_type == '4x404' && (in_array(2, $userDeskIds) || in_array(3, $userDeskIds) || in_array(4, $userDeskIds))))
                                <tr>
                                    <td>{{ $sl++ }}</td>
                                    <td>{{ date('d-M-Y h:i A', strtotime($inspectionInfo->inspection_report_date)) }}</td>
                                    <td>{{ date('d-M-Y h:i A', strtotime($inspectionInfo->created_at)) }}</td>
                                    ...
                                    <td>
                                        <a class="btn btn-info btn-sm" target="_blank" href="{{ url('/irc-recommendation-new/inspection-report-view/'.Encryption::encodeId($inspectionInfo->id)) }}">View</a>
                                    </td>
                                </tr>
                            @endif
                        @endforeach
