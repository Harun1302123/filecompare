...
class ImportPermissionController extends Controller
{
    ...
    public function appStore(Request $request)
    {
        ...
        if ($request->get('actionBtn') == 'searchBRinfo') {
            ...
            if ( $request->has('ref_app_tracking_no')) {
				...
				$masterData = $this->importPermissionService->handelMechineryData($getBrApprovedData->ref_id, $getBrApprovedData->process_type_id);
        ...
        try {
            ...
            if ($request->get('name')) {
                foreach ($request->get('name') as $key => $value) {
                    ...
                    $listOfMachineryImportedMaster = MasterMachineryImported::where('id', $masterID)->where('status', 1)->where('is_archive', 0)->where('is_deleted', 0) ->whereNotIn('amendment_type', ['delete', 'remove'])->whereRaw('quantity - total_imported - ? >= 0', [$requiredQty])->first();

                    if (empty($listOfMachineryImportedMaster)) {
                        Log::error("IPAppStore : Required quantity can't be greated then remaining quantity [IP-1059]");
                        Session::flash('error', "IPAppStore : Required quantity can't be greated then remaining quantity [IP-1059]");
                        return redirect()->back();
                    }
                }
            }
    ...
    public static function IpMachineryImportedApproved($ref_no, $ipRefAppTrackingNo)
    {
        try {
            ...
            if (count($listOfMachineryImportedSpare) > 0) {
                foreach ($listOfMachineryImportedSpare as $machineryImportedSpare) {
                    $listOfMachineryImportedMaster = MasterMachineryImported::where('id', $machineryImportedSpare->master_ref_id)->where('status', 1)->where('is_archive', 0)->where('is_deleted', 0) ->whereNotIn('amendment_type', ['delete', 'remove'])->whereRaw('quantity - total_imported > 0')->first();
                    $listOfMachineryImportedMaster->total_imported += $machineryImportedSpare->required_quantity;
