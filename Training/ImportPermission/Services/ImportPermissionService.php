...
//        }
    ...
    public function handelMechineryData($ref_id, $process_type){

        $colum_name = $process_type == 102 ? 'br_app_id' : 'bra_app_id';
        // $refBRAppTrackingNo = $getBRinfo->br_tracking_no;
        $machineryDataGetQry = MasterMachineryImported::where($colum_name, $ref_id)
        ->whereNotIn('amendment_type', ['delete', 'remove'])
        ->where('status', 1)
        ->where('is_archive', 0)
        ->where('is_deleted', 0);

        $listOfMachineryImported = $machineryDataGetQry->get();
        ...
        $listOfMachineryImportedMaster =  $machineryDataGetQry->whereRaw('quantity - total_imported > 0')->get(); 
