...
class BRCommonPoolManager
{
    ...
    public static function BRMachineryDataStore($tracking_no, $ref_id)
    {
        try {
            DB::beginTransaction();

            $listOfMachineryImported = ListOfMachineryImported::select('l_machinery_imported_name', 'l_machinery_imported_qty')
            ->where('app_id', $ref_id)
            ...
            if (count($listOfMachineryImported) > 0) {
                foreach ($listOfMachineryImported as $machineryImported) {
                    $listOfMachineryImportedMaster = new MasterMachineryImported();
                    $listOfMachineryImportedMaster->name = $machineryImported->l_machinery_imported_name;
                    $listOfMachineryImportedMaster->quantity = $machineryImported->l_machinery_imported_qty;
                    $listOfMachineryImportedMaster->total_imported = 0;
                    $listOfMachineryImportedMaster->amendment_type = 'add';
                    $listOfMachineryImportedMaster->ref_app_tracking_no = $tracking_no;
    ...
    public static function BRAMachineryDataStore($tracking_no, $ref_id)
    {
        try {
            DB::beginTransaction();
        
            $ref_app_tracking_no = BidaRegistrationAmendment::where('id', $ref_id)->value('ref_app_tracking_no');
            ...
            if (!empty($ref_app_tracking_no)) {
                $listOfMachineryImportedMaster = MasterMachineryImported::where('ref_app_tracking_no', $ref_app_tracking_no)->get();
            } else {
                $listOfMachineryImportedMaster = new MasterMachineryImported();
            }
            ...
            if (count($listOfMachineryImportedBRA) > 0) {
                foreach ($listOfMachineryImportedBRA as $machineryImportedBRA) {
                    ...
                    foreach ($listOfMachineryImportedMaster as $machineryImportedMaster) {        
                        if (MasterMachineryImported::processName($machineryImportedMaster->name) == MasterMachineryImported::processName($machineryImportedBRA->l_machinery_imported_name)) {
                            $found = true;
                    ...
                    if (!$found) {
                        ...
                        $machineryImportedMaster->ref_app_tracking_no = $ref_app_tracking_no;
                        $machineryImportedMaster->name = $machineryImportedBRA->l_machinery_imported_name;
                        $machineryImportedMaster->quantity = $machineryImportedBRA->l_machinery_imported_qty;
                        $machineryImportedMaster->total_imported = 0;
                        $machineryImportedMaster->amendment_type = $machineryImportedBRA->amendment_type;
                        $machineryImportedMaster->status = $machineryImportedBRA->status;
                        $machineryImportedMaster->save();
                    }
