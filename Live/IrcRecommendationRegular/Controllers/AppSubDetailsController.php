...
use App\Modules\IrcRecommendationRegular\Models\IrcSalesStatement;
...
class AppSubDetailsController extends Controller
{
    ...
    public function annualProductionCapacityForm($app_id)
    {
        ...
        return \view('IrcRecommendationRegular::annual_production_capacity.create-annual-production', compact('productUnit', 'app_id'));
    }
        ...
        try {
            ...
            if (!empty($app_id)) {
                foreach ($request->em_product_name as $proKey => $proData) {
                    $annualProduction = RegularAnnualProductionCapacity::findOrNew($request->apc_id);
            ...
            DB::rollback();
            Log::error('IRCRecommendationRegular : '.$e->getMessage().' '.$e->getFile().' '.$e->getLine().' [IRC-3-10022]');
            return response()->json([
                'error' => true,
    ...
    public function loadAannualProductionCapacityData(Request $request)
    {
        ...
        $getData = RegularAnnualProductionCapacity::leftJoin('product_unit', 'product_unit.id', '=','irc_regular_annual_production_capacity.quantity_unit')->where('app_id', $app_id)
            ->orderBy('irc_regular_annual_production_capacity.id', 'DESC')
            ->get([DB::raw('@rownum := @rownum+1 AS sl'), 'irc_regular_annual_production_capacity.*', 'product_unit.name as unit_name']);

                
        $html = strval(view("IrcRecommendationRegular::annual_production_capacity.load_apc_data", compact('getData')));
        return response()->json(['responseCode' => 1, 'html' => $html]);
    }

    public function annualProductionCapacityEditForm($app_id)
    {
        $apcData = RegularAnnualProductionCapacity::find(Encryption::decodeId($app_id));
        $productUnit = ['' => 'Select one'] + ProductUnit::where('status', 1)
            ->where('is_archive', 0)
            ->orderBy('name')
            ->lists('name', 'id')
            ->all();
        return \view('IrcRecommendationRegular::annual_production_capacity.edit-annual-production', compact('productUnit', 'app_id', 'apcData'));
    }

    public function annualProductionCapacityDelete($app_id)
    {
        $decoded_id = Encryption::decodeId($app_id);
        RegularAnnualProductionCapacity::where(['id' => $decoded_id])->delete();
    ...
    public function annualProductionCapacityInfo(Request $request)
    {
        ...
        $annualProduction = RegularAnnualProductionCapacity::select('id')->where('app_id', $app_id)->get();
        foreach ($annualProduction as $value) {
            $rawMeterial = RegularRawMaterial::select('app_id','apc_product_id')->where('app_id', $app_id)->where('apc_product_id', $value->id)->get();
    ...
    {
        $nationality = ['' => 'Select one'] + Countries::where('country_status', 'Yes')
            ->where('nationality', '!=', '')
            ->orderby('nationality', 'asc')
            ->lists('nationality', 'id')->all();
        $passport_nationalities = Countries::orderby('nationality')
            ->where('nationality', '!=', '')
        ...
        return view('IrcRecommendationRegular::director.create',
    ...
    public function storeVerifyDirector(Request $request)
    {
        ...
        if (!$request->ajax()) {
            return 'Sorry! this is a request without proper way. [IRC-R-1027]';
        }
        $rules = [];
        $messages = [];
        ...
        try {
            ...
            DB::rollback();
            Log::error('IRCRecommendationRegular : '.$e->getMessage().' '.$e->getFile().' '.$e->getLine().' [ASDC-1050]');
            return response()->json([
                'error' => true,
    ...
    public function loadListOfDirectiors(Request $request)
    {
        ...
        $html = strval(view("IrcRecommendationRegular::director.load-director-data", compact('getData')));
        return response()->json(['responseCode' => 1, 'html' => $html]);
    }
    ...
    public function directorEditForm($id)
    {
        ...
        return view("IrcRecommendationRegular::director.edit", compact('directorInfo', 'nationality'));
    }
    ...
    public function directorUpdate(Request $request)
    {
        try {
            ...
            $directorInfo->date_of_birth = (!empty($request->get('date_of_birth')) ? date('Y-m-d', strtotime($request->get('date_of_birth'))) : '');
            $directorInfo->gender = $request->gender;
            $directorInfo->l_director_designation = $request->l_director_designation;
            ...
            $directorInfo->date_of_expiry = (!empty($request->get('date_of_expiry')) ? date('Y-m-d', strtotime($request->get('date_of_expiry'))) : '');
            $directorInfo->save();
    ...
    public function rawMaterialForm($app_id, $id)
    {
        ...
        $annual_production_capacity = RegularAnnualProductionCapacity::where('id', $apc_product_id)->first(['unit_of_product', 'product_name']);
        $raw_material = RegularRawMaterial::where('apc_product_id', $apc_product_id)->get();
        $total_price = RegularRawMaterial::where('apc_product_id', $apc_product_id)->sum('price_taka');

        $productUnit = ['' => 'Select one'] + ProductUnit::where('status', 1)->where('is_archive', 0)->orderBy('name')->lists('name', 'id')->all();
        
        return \view('IrcRecommendationRegular::raw_material.create-raw-material',
    ...
    public function storeRawMaterial(Request $request)
    {
        ...
        try {
            ...
            if (!empty($app_id)) {
                $raw_material_ids = [];
                    ...
                    foreach ($request->product_name as $proKey => $proData) {
                        ...
                        $raw_material = RegularRawMaterial::findOrNew($raw_material_id);
                    ...
                    if (count($raw_material_ids) > 0) {
                        RegularRawMaterial::where('apc_product_id', $raw_material->apc_product_id)->whereNotIn('id',$raw_material_ids)->delete();
                    }

                    RegularAnnualProductionCapacity::where('app_id', $app_id)->where('id', $apc_product_id)
                        ->update([
            ...
            DB::rollback();
            Log::error('IRCRecommendationRegularAdhoc : '.$e->getMessage().' '.$e->getFile().' '.$e->getLine().' [IRC-3-10020]');
            return response()->json([
                'error' => true,
