...
{!! Form::open(array('url' => '/irc-recommendation-regular/store-raw-material','method' => 'post', 'class' => 'form-horizontal smart-form','id'=>'rawMaterialForm',
...
<div class="modal-body">
    ...
    <div class="table-responsive">
        <table id="rawMaterialTable" class="table table-striped table-bordered dt-responsive" cellspacing="0"width="100%">
            <thead>
            <tr>
                        ...
                        <td style="text-align: left;">
                            @if($inc==0)
                                <a class="btn btn-sm btn-primary addTableRows" title="Add more"
                                   onclick="addTableRowForIRC('rawMaterialTable', 'rawMaterialTableRow{{$inc}}');">
                                   <i class="fa fa-plus"></i></a>
                            @else
                                <a href="javascript:void(0);" class="btn btn-sm btn-danger removeRow"
                                   onclick="removeTableRawMaterial('rawMaterialTable','rawMaterialTableRow{{$inc}}');"> 
...
<script>
    ...
    $(document).on('blur', '.hundred-percentage', function() {
        formatedPercentageValue();
    });

    function formatedPercentageValue() {
        $('.hundred-percentage').each(function() {
            var num = parseFloat($(this).val());
            var cleanNum = isNaN(num) ? '' : num.toFixed(2);
            $(this).val(cleanNum);
        });
    }
    ...
    function countHundredPercentage(percentId) {
        ...
        if (total <= 100 && total >= 0) {
            var formatedTotal = (parseFloat(total)).toFixed(2);
            $('#total_percentage').val(formatedTotal);
        }
    }
