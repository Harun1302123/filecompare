...
class CsvUploadDownloadController extends Controller
{
    ...
    {
        $annual_product_name = AnnualProductionCapacity::where('id', Encryption::decodeId($apc_id))->first(['product_name']);
        return view("IrcRecommendationNew::excel.import", compact('app_id', 'apc_id', 'annual_product_name'));
    }
    ...
    public function uploadCsvFile(Request $request)
    {
        ...
        try {
            ...
            if (in_array($file_mime, $mimes)) {
                ...
                return redirect('/irc-recommendation-new/request/' . $filePath . '/' . $request->get('app_id') . '/' . $request->get('apc_id') . '/' . $request->get('unit_of_product'));
            } else {
            ...
            Log::error('IRCRawMaterialStore : '.$e->getMessage().' '.$e->getFile().' '.$e->getLine().' [IRCNCSV-100]');
            return response()->json([
                'error' => true,
    ...
    public function previewDataFromCsv($path, $app_id, $apc_id, $unit_of_product, Excel $excel)
    {
        ...
        try {
            ...
            return view("IrcRecommendationNew::excel.upload-request", compact('page_header', 'excelData', 'path', 'app_id', 'apc_id', 'unit_of_product', 'alterStatus'));
        } catch (\Exception $e) {
...
//                $total_price_bd += $request->get('price_bd')[$i];
            ...
            $total_price_bd = RawMaterial::where('app_id', $appId)->where('apc_product_id',$apcId)->sum('price_taka');

            AnnualProductionCapacity::where('app_id', $appId)->where('id', $apcId)
                ->update([
            ...
            return redirect('process/irc-recommendation-new/edit-app/'.Encryption::encodeId($appId).'/'.Encryption::encodeId($this->process_type_id));
        } catch (\Exception $e) {
            Log::error('IRCRawMaterialStore : '.$e->getMessage().' '.$e->getFile().' '.$e->getLine().' [IRCNCSV-102]');
            return response()->json([
                'error' => true,
                'status' => CommonFunction::showErrorPublic($e->getMessage()).' [IRCNCSV-102]'
            ]);
        }
    }
