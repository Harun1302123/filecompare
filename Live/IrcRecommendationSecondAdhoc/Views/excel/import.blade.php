...
{!! Form::open(array('url' => '/irc-recommendation-second-adhoc/upload-csv-file','method' => 'post', 'class' => 'form-horizontal', 'id' => 'batchUpload',
...
<div class="modal-body">
    <div class="row form-group" id="unit_of_product">
        <div class="col-md-6 {{$errors->has('unit_of_product') ? 'has-error': ''}}">
            ...
            <div class="col-md-8">
                {!! Form::select('unit_of_product', [1=>'1', 10=>'10', 100=>'100', 1000=>'1000'], '', [ "id" => 'unit_of_product_id', "placeholder" => "Select One", 'class' => 'form-control required input-md']) !!}
                {!! $errors->first('unit_of_product','<span class="help-block">:message</span>') !!}
                <span class="help-text text-danger hidden" id="unit_of_product_error">Unit of Product field is required</span>
            </div>
        </div>
    </div>
                    ...
                    <div class="row">
                        <div class="col-md-12 col-sm-12">
                            ...
                            <div style="text-align: center">
                                <a href="{{ asset('assets/csv-sample/sample.xlsx') }}" ref="noopener" target="_blank">Sample Sheet</a>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    $('form').submit(function() {
        ...
        var unit_of_product_val = $( "#unit_of_product option:selected" ).text();
        console.log(unit_of_product_val);
        if(unit_of_product_val > 0){
            return true;
        }
        else{
            $("#unit_of_product_id").addClass('error');
            $("#unit_of_product_error").removeClass('hidden');
            return false;
        }
    });
</script>