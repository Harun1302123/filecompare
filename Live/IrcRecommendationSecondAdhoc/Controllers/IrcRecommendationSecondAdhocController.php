...
use App\Modules\IrcRecommendationSecondAdhoc\Models\BusinessClass;
use App\Modules\IrcRecommendationSecondAdhoc\Models\IrcBrAnnualProductionCapacity;
use App\Modules\IrcRecommendationSecondAdhoc\Models\IrcProjectStatus;
use App\Modules\IrcRecommendationSecondAdhoc\Models\IrcPurpose;
...
use App\Modules\IrcRecommendationSecondAdhoc\Models\IrcTypes;
...
use App\Modules\IrcRecommendationSecondAdhoc\Models\SecondInspectionAnnualProductionSpareParts;
...
class IrcRecommendationSecondAdhocController extends Controller
{
    ...
    */
    ...
    {
        if (!$request->ajax()) {
            return 'Sorry! this is a request without proper way. [IRC-2-1001]';
        }
        ...
        if (!ACL::getAccsessRight($this->aclName, $mode)) {
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>You have no access right! Contact with system admin for more information. [IRC-2-971]</h4>"
            ]);
        }
        ...
        if (CommonFunction::checkEligibilityAndBiApps($working_company_id) != 1) {
            return response()->json([
                'responseCode' => 1,
                'html' => "<div class='btn-center'><h4 class='custom-err-msg'>Sorry! You have no approved Basic Information application for BIDA services. [IRC-2-9991]</h4> <br/> <a href='/dashboard' class='btn btn-primary btn-sm'>Apply for Basic Information</a></div>"
            ]);
        }
        ...
        if (in_array($department_id, [1, 4])) {
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>Sorry! The department is not allowed to apply to this application. [IRC-2-1041]</h4>"
            ]);
        }
        ...
        try {
            ...
            if (empty($payment_config)) {
                return response()->json([
                    'responseCode' => 1,
                    'html' => "<h4 class='custom-err-msg'> Payment Configuration not found ![IRC-2-10101]</h4>"
                ]);
            }
            ...
            if (empty($getCompanyData)) {
                return response()->json([
                    'responseCode' => 1,
                    'html' => "<h4 class='custom-err-msg'>Sorry! You have no approved Basic Information application. [IRC-2-9992]</h4>"
                ]);
            }
            ...
            $public_html = strval(view("IrcRecommendationSecondAdhoc::application-form", compact('mode', 'viewMode', 'IRCType', 'IRCPurpose', 'countriesWithoutBD', 'countries',
            ...
            Log::error('IRCAddForm : ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRC-2-1005]');
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>" . CommonFunction::showErrorPublic($e->getMessage()) . ' [IRC-2-1005]' . "</h4>"
            ]);
        }
    }
    ...
    public function getDocList(Request $request)
    {
        ...
        $html = strval(view("IrcRecommendationSecondAdhoc::documents",
    ...
    public function appStore(Request $request)
    {
        ...
        if (!ACL::getAccsessRight($this->aclName, $mode, $app_id)) {
            abort('400', 'You have no access right! Contact with system admin for more information. [IRC-2-972]');
        }

        // Check application category is valid or not
        $getIrcType = IrcTypes::find($this->app_type_id);
        if (empty($getIrcType)) {
            Session::flash('error', "Unknown Irc type! [IRC-2-1211]");
        ...
        if ($request->get('actionBtn') == 'searchBRinfo') {
            ...
            if ($request->get('last_br_check') == 'yes' && $request->has('br_ref_app_tracking_no')) {
                ...
                if (empty($getBRapprovedData)) {
                    Session::flash('error', 'Sorry! BIDA Registration not found by tracking no! [IRC-2-111]');
                    return redirect()->back();
                }

                $getBRinfo = UtilFunction::checkBRCommonPoolData($getBRapprovedData->tracking_no, $getBRapprovedData->ref_id);
                if (empty($getBRinfo)) {
                    Session::flash('error', 'Sorry! BIDA Registration not found by tracking no! [IRC-2-1081]');
                    return redirect()->back();
                }
                ...
                // Load BR information
                    ...
                    $BRListOfDirectors = ListOfDirectors::where('app_id', $getBRapprovedData->ref_id)->where('process_type_id', 102)->where('status',1)->where('is_archive',0)->get();
                ...
                if (Session::has('loadData')) {
                    if (Session::get('loadData.last_br') == 'yes' && Session::get('loadData.br_ref_app_tracking_no') != $request->get('br_ref_app_tracking_no')) {
                        Session::flash('error', 'Sorry! IRC 1st adhoc BIDA Registration tracking no did not match! [IRCFA-113]');
                        return redirect()->back();
                    }
        ...
        if ($request->get('actionBtn') == 'searchIRCinfo') {
            ...
            // if applicant have approved IRC 1st adhoc tracking no given then set session
            if ($request->get('last_irc_check') == 'yes' && $request->has('irc_ref_app_tracking_no')) {
                ...
                $getIRCFAApprovedRefInfo = ProcessList::where('tracking_no', $refAppTrackingNo)
                    ...
                    ->where('process_type_id', 13)
                    ->first(['ref_id', 'tracking_no', 'approval_center_id']);

                if (empty($getIRCFAApprovedRefInfo)) {
                    Session::flash('error', 'Sorry! approved IRC 1st adhoc reference no. is not found or not allowed! [IRCFA-111]');
                    return redirect()->back();
                }

                $getIRCinfo = UtilFunction::checkIRCCommonPoolData($getIRCFAApprovedRefInfo->tracking_no, $getIRCFAApprovedRefInfo->ref_id);
                if (empty($getIRCinfo)) {
                    Session::flash('error', 'Sorry! IRC 1st adhoc not found by tracking no! [IRCFA-112]');
                    return redirect()->back();
                }
                ...
                Session::flash('success', 'Successfully loaded IRC 1st adhoc data. Please proceed to next step');
                return redirect()->back();
            }
        }
        ...
        if (in_array($department_id, [1, 4])) {
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>Sorry! The department is not allowed to apply to this application. [IRC-2-1042]</h4>"
            ]);
        }

        if (CommonFunction::checkEligibilityAndBiApps($working_company_id) != 1) {
            Session::flash('error', "Sorry! You have no approved Basic Information application for BIDA services. [IRC-2-9993]");
            return redirect()->back();
        }
        ...
        if (!$payment_config) {
            Session::flash('error', "Payment configuration not found [IRC-2-101]");
        ...
        if ($stakeDistribution->isEmpty()) {
            Session::flash('error', "Stakeholder not found [IRC-2-100]");
            return redirect()->back()->withInput();
        }

        //  Required Documents for attachment
        $attachment_key = $getIrcType->attachment_key;
...
//        }
        ...
        try {
            ...
            if ($request->get('app_id')) {
                $appData = IrcRecommendationSecondAdhoc::find($app_id); 
                $processData = ProcessList::where([
                    'process_type_id' => $this->process_type_id, 'ref_id' => $appData->id
                ])->first();
            } else {
                $appData = new IrcRecommendationSecondAdhoc();
            ...
            $appData->last_irc_1st_adhoc = $request->get('last_irc_check');
            ...
            if ($request->hasFile('irc_ccie_brows_copy')) {
                ...
                $file_path = trim(uniqid('IRC_2nd_CCIE_COPY' . '-', true) . $_file_path->getClientOriginalName());
            ...
            if ($request->has('business_class_code')) {
                ...
                if (empty($get_business_class['data'])) {
                    Session::flash('error', "Sorry! Your given Code of business class is not valid. Please enter the right one. [IRC-2-1017]");
                    return redirect()->back();
                }
            ...
            if (!empty($request->investor_signature_base64)) {
                ...
                $investor_signature_name = trim(sprintf("%s", uniqid('BIDA_IRC_2nd_', true))) . '_' . time() . '.jpeg';
            ...
            if ($appData->irc_purpose_id != 2 && !empty($appData->id) && !empty($request->get('apc_product_name')[0])) {
                ...
                foreach ($request->apc_product_name as $proKey => $proData) {
                    $annualCap = new SecondAnnualProductionCapacity();
                    ...
                    if(count(Session::get('loadIRCAnnualProductionCapacity')) > 0) {
                        ...
                        //get first adhoc raw material ...
                        $firstRawMaterialData = RawMaterial::where('apc_product_id', $request->apcId[$proKey])->get();

                        //Store Second adhoc raw material ...
                        if (!empty($firstRawMaterialData)) {
                            foreach ($firstRawMaterialData as $data) {
                                $secondRawMaterialData = new SecondRawMaterial();
                                $secondRawMaterialData->app_id = $appData->id;
                                $secondRawMaterialData->apc_product_id = $annualCap->id;
                                $secondRawMaterialData->product_name = $data->product_name;
                                $secondRawMaterialData->hs_code = $data->hs_code;
                                $secondRawMaterialData->quantity = $data->quantity;
                                $secondRawMaterialData->quantity_unit = $data->quantity_unit;
                                $secondRawMaterialData->price_taka = $data->price_taka;
                                $secondRawMaterialData->percent = $data->percent;
                                $secondRawMaterialData->save();
                            }
                        }
                    }
            ...
            if ($appData->irc_purpose_id != 1 && !empty($appData->id) && !empty($request->get('apsp_product_name')[0])) {
                ...
                foreach ($request->apsp_product_name as $proKey => $proData) {
                    ...
                    $annualSpare = SecondAnnualProductionSpareParts::findOrNew($apsp_id);
                ...
                if (count($apsp_ids) > 0) {
                    SecondAnnualProductionSpareParts::where('app_id', $appData->id)->whereNotIn('id', $apsp_ids)->delete();
                }
            }
            ...
            if (!empty($appData->id)) {
                ...
                foreach ($request->get('country_id') as $key => $value) {
                    ...
                    $source_of_finance = SecondIrcSourceOfFinance::findOrNew($source_of_finance_id);
                ...
                if (count($source_of_finance_ids) > 0) {
                    SecondIrcSourceOfFinance::where('app_id', $appData->id)->whereNotIn('id', $source_of_finance_ids)->delete();
                }
            }
            ...
            if (!empty($appData->id)) {
                ...
                foreach ($request->other_licence_name as $key => $value) {
                    ...
                    $otherLicence = SecondIrcOtherLicenceNocPermission::findOrNew($source_of_finance_id);
                    ...
                    $otherLicence->issue_date = (!empty($request->get('other_licence_issue_date')[$key]) ? date('Y-m-d',
                        strtotime($request->get('other_licence_issue_date')[$key])) : '');
                    $otherLicence->save();
                    $other_licence_ids[] = $otherLicence->id;
                }
                if (count($other_licence_ids) > 0) {
                    SecondIrcOtherLicenceNocPermission::where('app_id', $appData->id)->whereNotIn('id', $other_licence_ids)->delete();
                }
            }
            ...
            if (!empty($appData->id) && !empty($request->get('sales_statement_date')[0])) {
                $sales_statement_ids = [];
//                dd($request->get('sales_statement_date'));
...
//            }
            ...
            */
            if ($request->get('actionBtn') == 'Submit' && $processData->status_id == -1) {
                ...
                //     Session::flash('error', "Sorry! You already have pending application in processing. [IRC-2-1430]");
                //     return redirect()->back();
                // }

                if (empty($processData->tracking_no)) {
                    $prefix = 'IRC-2-' . date("dMY") . '-';
            ...
            if ($processData->status_id == -1) {
                ...
                Session::flash('error', 'Failed due to Application Status Conflict. Please try again later! [IRC-2-1023]');
            }

            DB::commit();
            return redirect('irc-recommendation-second-adhoc/list/' . Encryption::encodeId($this->process_type_id));
        } catch (\Exception $e) {
            //dd($e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine());
            DB::rollback();
            Log::error('IRCAppStore : ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRC-2-1060]');
            Session::flash('error', CommonFunction::showErrorPublic($e->getMessage()) . ' [IRC-2-1060]');
            return redirect()->back()->withInput();
        }
    }
    ...
    {
        if (!$request->ajax()) {
            return 'Sorry! this is a request without proper way. [IRC-2-1002]';
        }
        ...
        if (!ACL::getAccsessRight($this->aclName, $mode)) {
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>You have no access right! Contact with system admin for more information. [IRC-2-973]</h4>"
            ]);
        }
        ...
        if (in_array($department_id, [1, 4])) {
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>Sorry! The department is not allowed to apply to this application. [IRC-2-1043]</h4>"
            ]);
        }
        ...
        try {
            ...
            $annualProductionCapacity = SecondAnnualProductionCapacity::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_annual_production_capacity.quantity_unit')
                ...
                ->get([
                    'irc_2nd_annual_production_capacity.*',
                    'product_unit.name as unit_name'
                ]);

            $annualProductionSpareParts = SecondAnnualProductionSpareParts::where('app_id', $applicationId)->get();
            $eaOwnershipStatus = ['' => 'Select one'] + EA_OwnershipStatus::where('is_archive', 0)->where('status',
                    1)->orderBy('name')->lists('name', 'id')->all();
            $eaOrganizationStatus = ['' => 'Select one'] + EA_OrganizationStatus::where('is_archive',
                    0)->where('status', 1)->orderBy('name')->lists('name', 'id')->all();
            ...
            $source_of_finance = SecondIrcSourceOfFinance::where('app_id', $applicationId)->get();
            ...
            $salesStatement = IrcSalesStatement::where('app_id', $applicationId)->get();
            $otherLicence = SecondIrcOtherLicenceNocPermission::where('app_id', $applicationId)->get();
            $approvalCenterList = DivisionalOffice::where('status', 1)
                ->where('is_archive', 0)
                ->orderBy('id')
                ->get([
            ...
            $public_html = strval(view("IrcRecommendationSecondAdhoc::application-form-edit",
            ...
            Log::error('IRCViewEditForm : ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRC-2-1010]');
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>" . CommonFunction::showErrorPublic($e->getMessage()) . "[IRC-2-1010]" . "</h4>"
            ]);
        }
    }
    ...
    public function applicationView($app_id, Request $request)
    {
        ...
        try {
            ...
            $source_of_finance = SecondIrcSourceOfFinance::leftJoin('country_info', 'country_info.id', '=', 'irc_2nd_source_of_finance.country_id')
                ->where('app_id', $applicationId)
                ->get([
                    'irc_2nd_source_of_finance.equity_amount',
                    'irc_2nd_source_of_finance.loan_amount',
            ...
            $annualProductionCapacity = SecondAnnualProductionCapacity::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_annual_production_capacity.quantity_unit')
                ->where('irc_2nd_annual_production_capacity.app_id', $applicationId)
                ->get([
                    'irc_2nd_annual_production_capacity.id as apc_id',
                    'irc_2nd_annual_production_capacity.product_name',
                    'product_unit.name',
                    'irc_2nd_annual_production_capacity.price_usd',
                    'irc_2nd_annual_production_capacity.quantity',
                    'irc_2nd_annual_production_capacity.price_taka',
                ]);
            ...
            $annualProductionSpareParts = SecondAnnualProductionSpareParts::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_annual_production_spare_parts.quantity_unit')
                ->where('irc_2nd_annual_production_spare_parts.app_id', $applicationId)
                ->get([
                    'irc_2nd_annual_production_spare_parts.product_name',
                    'product_unit.name',
                    'irc_2nd_annual_production_spare_parts.quantity',
                    'irc_2nd_annual_production_spare_parts.price_usd',
                    'irc_2nd_annual_production_spare_parts.price_taka',
                ]);

            $IRCSalesStatements = IrcSalesStatement::where('app_id', $applicationId)->where('process_type_id', $this->process_type_id)->get();
          ...
          ");
            ...
            $inspectionInfo = SecondIrcInspection::where('app_id', $appInfo->id)->orderBy('inspection_report_date', 'desc')
                ->get([
                    'io_name',
                    'id',
            ...
            $last_inspection_id = SecondIrcInspection::where('app_id', $appInfo->id)->where('ins_approved_status', 1)->value('id');
            ...
            $public_html = strval(view("IrcRecommendationSecondAdhoc::application-form-view",
            ...
            Log::error('IRCView : ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRC-2-10111]');
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>" . CommonFunction::showErrorPublic($e->getMessage()) . "[IRC-2-10111]" . "</h4>"
            ]);
        }
    }
    ...
    {
        return view("IrcRecommendationSecondAdhoc::preview");
    }

    public function uploadDocument()
    {
        return View::make('IrcRecommendationSecondAdhoc::ajaxUploadFile');
    }
    ...
    public function getBusinessClassSingleList(Request $request)
    {
        ...
        $sub_class = BusinessClass::select('id', DB::raw("CONCAT(code, ' - ', name) as name"))->where('pare_code', $business_class_code)
    ...
    {
        if (!$request->ajax()) {
            return 'Sorry! this is a request without proper way. [IRC-2-1022]';
        }

        return view("IrcRecommendationSecondAdhoc::business-class-modal");
    }
    ...
    public function afterPayment($payment_id)
    {
        ...
        try {
            ...
            if ($paymentInfo->payment_category_id == 1) {
                if ($processData->status_id != '-1') {
                    Session::flash('error', 'This is an invalid status, it\'s not possible to get the next status. [IRC-2-911]');
                    return redirect('process/irc-recommendation-second-adhoc/edit-app/' . Encryption::encodeId($processData->ref_id) . '/' . Encryption::encodeId($processData->process_type_id));
                }
...
//            }
            ...
            return redirect('process/irc-recommendation-second-adhoc/view-app/' . Encryption::encodeId($processData->ref_id) . '/' . Encryption::encodeId($processData->process_type_id));
        } catch (\Exception $e) {
            DB::rollback();
            Log::error('IRCAfterPayment: ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRC-2-102]');
            Session::flash('error', CommonFunction::showErrorPublic($e->getMessage()) . ' [IRC-2-102]');
            return redirect('process/irc-recommendation-second-adhoc/edit-app/' . Encryption::encodeId($processData->ref_id) . '/' . Encryption::encodeId($processData->process_type_id));
        }
    }
...
//                $processData->desk_id = 1;
            ...
            DB::rollback();
            Log::error('IRCAfterCounterPayment: ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRC-2-103]');
            Session::flash('error', CommonFunction::showErrorPublic($e->getMessage()) . ' [IRC-2-103]');
            return redirect('process/irc-recommendation-second-adhoc/edit-app/' . Encryption::encodeId($processData->ref_id) . '/' . Encryption::encodeId($processData->process_type_id));
        }
    }
    ...
    public function getRawMaterial($id)
    {
        ...
        $raw_material = SecondRawMaterial::where('apc_product_id', $id)
            ->get([DB::raw('@rownum := @rownum+1 AS sl'), 'irc_2nd_raw_material.*']);

        $productUnit = [0 => ''] + ProductUnit::where('status', 1)->where('is_archive', 0)->orderBy('name')->lists('name', 'id')->all();

        //total raw material price
        $total_value = SecondAnnualProductionCapacity::where('id', $id)->first(['unit_of_product', 'raw_material_total_price']);

        return \view('IrcRecommendationSecondAdhoc::raw_material.raw-material-modal', compact('raw_material', 'total_value', 'productUnit'));
    }
    ...
    public function inspectionForm($inspectionId)
    {
        ...
        try {
            ...
            $inspectionInfo = ProcessList::leftJoin('irc_2nd_inspection as apps', 'apps.app_id', '=', 'process_list.ref_id')
                ->leftJoin('user_desk', 'user_desk.id', '=', 'process_list.desk_id')
                ->leftJoin('process_status as ps', function ($join) use ($process_type_id) {
                    $join->on('ps.id', '=', 'process_list.status_id');
                    $join->on('ps.process_type_id', '=', DB::raw($process_type_id));
            ...
            if (!($inspectionInfo->created_by == Auth::user()->id || in_array(Auth::user()->user_type, ['1x101', '2x202']) || (Auth::user()->user_type == '4x404' && (in_array(2, $userDeskIds) || in_array(3, $userDeskIds) || in_array(4, $userDeskIds))))) {
                return response()->json([
                    'responseCode' => 1,
                    'html' => "<h4 class='custom-err-msg'>You have no access right! Contact with system admin for more information. [IRC-2-974]</h4>"
                ]);
            }

            $annualProductionCapacity = SecondAnnualProductionCapacity::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_annual_production_capacity.quantity_unit')
                ->where('irc_2nd_annual_production_capacity.app_id', $inspectionInfo->app_id)
                ->where('irc_2nd_annual_production_capacity.status', 1)
                ->where('irc_2nd_annual_production_capacity.is_archive', 0)
                ->get(['product_unit.name as unit_name', 'irc_2nd_annual_production_capacity.product_name', 'irc_2nd_annual_production_capacity.quantity']);

            $annualProductionSpareParts = SecondAnnualProductionSpareParts::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_annual_production_spare_parts.quantity_unit')
                ->where('irc_2nd_annual_production_spare_parts.app_id', $inspectionInfo->app_id)
                ->where('irc_2nd_annual_production_spare_parts.status', 1)
                ->where('irc_2nd_annual_production_spare_parts.is_archive', 0)
                ->get(['product_unit.name as unit_name', 'irc_2nd_annual_production_spare_parts.product_name', 'irc_2nd_annual_production_spare_parts.quantity']);

            $inspectionAnnualProductionCapacity = SecondInspectionAnnualProduction::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_inspection_annual_production.quantity_unit')
                ->where('irc_2nd_inspection_annual_production.inspection_id', $inspectionInfo->id)
                ->get(['product_unit.name as unit_name', 'irc_2nd_inspection_annual_production.*']);

            $inspectionAnnualProductionSpareParts = SecondInspectionAnnualProductionSpareParts::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_inspection_annual_production_spare_parts.quantity_unit')
                ->where('irc_2nd_inspection_annual_production_spare_parts.inspection_id', $inspectionInfo->id)
                ->get(['product_unit.name as unit_name', 'irc_2nd_inspection_annual_production_spare_parts.*']);

            $totalFee = DB::table('irc_inspection_gov_fee_range')->where('status', 1)->get();

            return view('IrcRecommendationSecondAdhoc::inspection-report-form', compact('inspectionInfo', 'inspectionAnnualProductionCapacity', 'annualProductionCapacity', 'annualProductionSpareParts', 'inspectionAnnualProductionSpareParts',
                'initialProduction', 'listOfMachineryImportedTotal', 'listOfMachineryLocalTotal', 'totalFee'));

        } catch (\Exception $e) {
            Log::error('IRCViewInspectionForm : ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRC-2-1010]');
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>" . CommonFunction::showErrorPublic($e->getMessage()) . "[IRC-2-1010]" . "</h4>"
            ]);
    ...
    public function inspectionReportGenerate($inspectionId)
    {
        try {
            ...
            $contents = view('IrcRecommendationSecondAdhoc::inspection-report-generate', compact('inspectionInfo', 'inspectionAnnualProductionCapacity',
                'inspectionAnnualProductionSpareParts', 'annualProductionCapacity', 'annualProductionSpareParts', 'initialProduction', 'listOfMachineryImportedTotal', 'listOfMachineryLocalTotal'))->render();
            ...
            Log::error('IRCViewInspectionForm : ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRC-2-1070]');
            return response()->json([
                'responseCode' => 1,
                'html' => "<h4 class='custom-err-msg'>" . CommonFunction::showErrorPublic($e->getMessage()) . "[IRC-2-1010]" . "</h4>"
            ]);
    ...
    public function entitlementPaperPDFold($inspectionId)
    {
        try {
            ...
            $inspectionProductionCapacity = SecondInspectionAnnualProduction::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_inspection_annual_production.quantity_unit')
                ->where('irc_2nd_inspection_annual_production.inspection_id', $inspection_id)
                ->get([
                    'product_unit.name as unit_name',
                    'irc_2nd_inspection_annual_production.*'
                ]);

            $inspectionProductionSpare = SecondInspectionAnnualProductionSpareParts::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_inspection_annual_production_spare_parts.quantity_unit')
                ->where('irc_2nd_inspection_annual_production_spare_parts.inspection_id', $inspection_id)
                ->get([
                    'product_unit.name as unit_name',
                    'irc_2nd_inspection_annual_production_spare_parts.*'
                ]);

            $contents = view('IrcRecommendationSecondAdhoc::inspection-entitlement-pdf', compact('inspectionInfo', 'inspectionProductionCapacity', 'inspectionProductionSpare'))->render();
    ...
    public function entitlementPaperPDF($inspectionId)
    {
        ...
        try {
            ...
            $inspectionInfo = ProcessList::leftJoin('irc_2nd_inspection as apps', 'apps.app_id', '=', 'process_list.ref_id')
                ->leftJoin('irc_project_status', 'irc_project_status.id', '=', 'apps.project_status_id')
                ->leftjoin('irc_2nd_apps', 'irc_2nd_apps.id', '=', 'apps.app_id')
                ->where('apps.id', $inspection_id)
                ->where('process_list.process_type_id', $this->process_type_id)
                ->where('apps.ins_approved_status', 1)
                ->where('process_list.status_id', 25)
                ->first([
                    'process_list.tracking_no',
                    'irc_project_status.name as project_status_name',
                    'irc_2nd_apps.g_full_name',
                    'irc_2nd_apps.g_designation',
                    'irc_2nd_apps.g_signature',
                    'apps.*'
    ...
    public function ProductionCapacityPDF($app_id)
    {
        try {
            ...
            $lastInspectionInfo = ProcessList::leftJoin('irc_2nd_inspection as apps', 'apps.app_id', '=', 'process_list.ref_id')
            ...
            $inspectionAnnualProductionSpareParts = SecondInspectionAnnualProductionSpareParts::leftJoin('product_unit', 'product_unit.id', '=', 'irc_2nd_inspection_annual_production_spare_parts.quantity_unit')
                ->where('irc_2nd_inspection_annual_production_spare_parts.inspection_id', $lastInspectionInfo->id)
                ->get(['product_unit.name as unit_name', 'irc_2nd_inspection_annual_production_spare_parts.*']);

            $productUnit = [0 => ''] + ProductUnit::where('status', 1)->where('is_archive', 0)->orderBy('name')->lists('name', 'id')->all();

            $contents = view('IrcRecommendationSecondAdhoc::inspection-production-capacity-pdf',
    ...
    public function listOfAnnualProduction(Request $request)
    {
        ...
        $total_list_of_apc = SecondAnnualProductionCapacity::where('app_id', $app_id)->count();
